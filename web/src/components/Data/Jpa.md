在上一篇[Spring中使用JdbcTemplate访问数据库](/Data/JdbcTemplate) 中介绍了一种基本的数据访问方式，结合[构建RESTful API](/Web/RESTfulAPI)和[使用Thymeleaf模板引擎渲染Web视图](/Web/Thymeleaf)的内容就已经可以完成App服务端和Web站点的开发任务了。
